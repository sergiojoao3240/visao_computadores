#include <stdio.h>
#include "vc.h"
// >>>>>>>>>>>>>>>> Segmentação de uma imagem binaria com o método midpoint <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
int main(void)
{
    ...
    image = vc_read_image("IMG_Test_Segm/cells.pgm");
    ...
    image2 = vc_image_new(image->width,image->height,1,255); // niveis 1 ou 255?

    // chamamos a nossa funçao
    vc_gray_to_binary_bernsen(image, image2, 25, 15);



    //função dada, nome de imagem e o respetivo apontador
    vc_write_image("img_seg_bernsen_cells.pgm", image2);
